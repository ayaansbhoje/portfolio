...
export function DeskClock({ accent }: { accent: string }) {
  ...
  return (
    <group position={[1.5, DESK_TOP, -0.55]} rotation={[-0.15, -0.5, 0]}>
      ...
      {/* time + date as real 3D text → depth-tested, never floats over desk objects */}
      <Text position={[0, 0.42, 0.08]} fontSize={0.2} color={accent} anchorX="center" anchorY="middle" letterSpacing={0.06} toneMapped={false}>
        {disp}
      </Text>
      <Text position={[0, 0.23, 0.08]} fontSize={0.075} color={accent} fillOpacity={0.7} anchorX="center" anchorY="middle" letterSpacing={0.25} toneMapped={false}>
        {date}
      </Text>
    </group>
  );
